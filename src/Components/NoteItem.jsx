...
const NoteItem = ({ note, onToggleComplete, onDeleteNote, onUpdateNote, editingNoteId, setEditingNoteId, onOpenReminderModal }) => {
  ...
  const CategoryIcon = categoryDetails ? { CheckSquare, Lightbulb, FileText }[categoryDetails.icon] : FileText;
    ...
    return (
      <div className="bg-gray-700 rounded-lg p-4 ring-2 ring-blue-500">
        ...
        <div className="flex items-center justify-end space-x-2 mt-2">
          <button onClick={() => setEditingNoteId(null)} className="px-3 py-1 text-sm text-gray-400 hover:text-white transition-colors">Cancel</button>
          <button onClick={handleSave} className="px-3 py-1 text-sm bg-blue-600 rounded hover:bg-blue-500 transition-colors">Save</button>
        </div>
      </div>
    );
  }

  // Otherwise, show the normal note display
  return (
